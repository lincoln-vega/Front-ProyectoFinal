...
export default function HorariosSalonesPage() {
  ...
  const scheduleItems = useMemo(() => coursesList
    ...
    .filter((course) => {
      ...
      return !term || `${course.asignatura} ${course.docente} ${course.sala}`.toLowerCase().includes(term);
    })
    ...
    <div className="space-y-6">
      <div className="bg-white p-5 rounded-xl shadow-sm border border-slate-200 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Todas las clases organizadas por día, hora, docente y sala.</p>
        </div>
              ...
              return <div key={day} className="relative border-l border-slate-200" style={{ height: `${hours.length * HOUR_HEIGHT}px` }}>
                ...
                {dayItems.map((item, index) => {
                  ...
                  return <div key={`${item.id}-${day}-${index}`} className="absolute left-1.5 right-1.5 rounded-lg border-l-4 border-blue-600 bg-blue-50 p-2 shadow-xs overflow-hidden" style={{ top: `${top + 4}px`, height: `${height}px` }}>
                    <p className="truncate text-[11px] font-bold text-blue-950">{item.asignatura}</p>
                    <p className="truncate text-[10px] text-blue-800">{item.docente}</p>
                    <p className="mt-1 flex items-center gap-1 truncate text-[9px] text-slate-500"><Clock className="h-3 w-3 shrink-0" />{item.horario.replace(/^.*?\s(?=\d)/, "")}</p>
                    <p className="flex items-center gap-1 truncate text-[9px] text-slate-500"><MapPin className="h-3 w-3 shrink-0" />{item.sala}</p>
                  </div>;
                })}
              </div>;
            })}
