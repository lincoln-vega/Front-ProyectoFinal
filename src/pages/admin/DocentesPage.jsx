...
export default function DocentesPage() {
  ...
  useEffect(() => {
    const savedDocentes = localStorage.getItem("docentesData");
    if (savedDocentes) {
      setDocentes(JSON.parse(savedDocentes));
    } else {
      ...
      const initialData = [
        ...
        },
        {
          ...
          salario: 3200,
          incidencias: [
            { id: "i1", fecha: new Date().toLocaleDateString(), descripcion: "Falta por salud" }
          ],
          estado: "Activo"
        }
      ];
      setDocentes(initialData);
      localStorage.setItem("docentesData", JSON.stringify(initialData));
    }
  }, []);

  const saveToStorage = (data) => {
    setDocentes(data);
    localStorage.setItem("docentesData", JSON.stringify(data));
  };
