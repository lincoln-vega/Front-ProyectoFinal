...
export default function MatriculasPagosPage() {
  const [tabActiva, setTabActiva] = useState("matriculas"); // 'matriculas' | 'pagos'
  ...
  // Restablecer Datos
  const handleReset = () => {
    if (window.confirm("¿Deseas restablecer los datos de matrículas y pagos a los iniciales?")) {
      saveMatriculasStorage(INITIAL_MATRICULAS);
      savePagosStorage(INITIAL_PAGOS);
      showNotification("Datos iniciales de prueba restaurados.");
    }
  };
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center space-x-2 px-2.5 py-1 rounded bg-blue-50 text-[#1E3A8A] text-xs font-bold uppercase tracking-wider border border-blue-200 mb-2">
              ...
              <span>Módulo Financiero y Admisión</span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              onClick={handleReset}
              className="inline-flex items-center space-x-1.5 px-3 py-2 border border-slate-300 text-slate-700 bg-white hover:bg-slate-50 rounded-xl text-xs font-semibold shadow-2xs transition-colors cursor-pointer"
            ...
            <button
              onClick={() => setModalMatriculaOpen(true)}
